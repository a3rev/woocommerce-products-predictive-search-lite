...
class Admin_UI
{
	...
	public $framework_version      = '2.7.1';
	...
	public $plugin_prefix = WOOPS_PREFIX;
	...
	public function __construct() {
		$this->google_api_key_option     = WOOPS_KEY . '_google_api_key';
		$this->google_map_api_key_option = WOOPS_KEY . '_google_map_api_key';
		$this->toggle_box_open_option    = WOOPS_KEY . '_toggle_box_open';
		$this->version_transient         = WOOPS_KEY . '_licinfo';

		if ( defined( 'WOOPS_G_FONTS' ) ) {
			$this->is_load_google_fonts = (boolean) WOOPS_G_FONTS;
		}
	...
	public function get_premium_video_data() {
		$premium_video_data = array(
				'box_title'    => __( 'Premium Version Enhanced Features', 'woocommerce-predictive-search' ),
				'image_url'    => WOOPS_IMAGES_URL. '/video.jpg',
				'video_url'    => 'https://www.youtube.com/embed/9dGw-ORfMIk?version=3&autoplay=1',
				'left_title'   => __( 'Premium Version Enhanced Features', 'woocommerce-predictive-search' ),
				'left_text'    => __( 'WooCommerce Predictive Search Premium', 'woocommerce-predictive-search' )
				...
				'right_title'  => __( 'Developer Support and Premium Features', 'woocommerce-predictive-search' ),
				'right_text'   => __( 'Limited Time Offer. Purchase the Premium Version Lifetime License. That is a Lifetime of maintenance updates, feature upgrades and developer support for a once only fee. Offer ending soon.', 'woocommerce-predictive-search' )
	...
	public function upgrade_top_message( $echo = false, $setting_id = '' ) {
		$upgrade_top_message = sprintf( '<div class="pro_feature_top_message">' 
			. __( 'Advanced Settings - Upgrade to the <a href="%s" target="_blank">%s License</a> to activate these settings.', 'woocommerce-predictive-search' ) 
			. '</div>'
			...
			, apply_filters( $this->plugin_name . '_' . $setting_id . '_pro_version_name', apply_filters( $this->plugin_name . '_pro_version_name', __( 'Pro Version', 'woocommerce-predictive-search' ) ) )
		);
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'woocommerce-predictive-search' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'woocommerce-predictive-search' ), $this->support_url );
			} else {
